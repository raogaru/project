...
static backslashResult exec_command_trace(PsqlScanState scan_state, bool active_branch);
static backslashResult exec_command_color(PsqlScanState scan_state, bool active_branch);
				...
				PQExpBuffer previous_buf)
{
	...
	{
		if (pset.color) {
		pg_log_error("\033[0,31mInvalid command\033[0,0m \\%s", cmd);
		} else {
			 ...
			 PQExpBuffer previous_buf)
{
	...
	if (strcmp(cmd, "a") == 0)
		...
		status = exec_command_trace(scan_state, active_branch);
	else if (strcmp(cmd, "color") == 0)
		status = exec_command_color(scan_state, active_branch);
	...
	if (active_branch)
	{
		...
		if (!opt0)
		{
			...
			static const char *const my_list[] = {
				"border", "color", "columns", "csv_fieldsep", "expanded", "fieldsep",
				"fieldsep_zero", "footer", "format", "linestyle", "null",
				"numericlocale", "pager", "pager_min_lines",
				"recordsep", "recordsep_zero",
				"tableattr", "timing", "title", "trace", "tuples_only",
...
/*
 * \trace -- enable/disable trace of queries
 */
static backslashResult
exec_command_trace(PsqlScanState scan_state, bool active_branch)
{
	bool		success = true;
	...
	if (active_branch)
	{
		...
		if (opt)
			success = ParseVariableBool(opt, "\\trace", &pset.trace);
		else
			pset.trace = !pset.trace;
		if (!pset.quiet)
		{
			if (pset.trace)
				puts(_("Trace is on."));
...
/*
 * \color -- enable/disable colored output response for commands 
 */
static backslashResult
exec_command_color(PsqlScanState scan_state, bool active_branch)
{
	bool		success = true;
	...
	if (active_branch)
	{
		...
		if (opt)
			success = ParseVariableBool(opt, "\\color", &pset.color);
		else
			pset.color = !pset.color;
		if (!pset.quiet)
		{
			if (pset.color)
				puts(_("Color is on."));
