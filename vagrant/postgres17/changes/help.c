...
#define VALUE_OR_NULL(a) ((a) ? (a) : "")
	...
	if (!topic || strlen(topic) == 0)
	{
		...
		output = PageOutput(nrows + 1, pager ? &(pset.popt.topt) : NULL);
               ...
               fputs(_("RaogaruDB Command Help available for:\n"), output);
		...
		for (i = 0; i < nrows; i++)
		{
			...
			for (j = 0; j < ncolumns - 1; j++)
                             fprintf(output, "%3d.%-*s", (nrows*j)+i+1,
                                                QL_MAX_CMD_LEN + 1,
			...
			for (i = 0; QL_HELP[i].cmd; i++)
			{
					...
					strcmp(topic, "*") == 0)
				{
					...
					QL_HELP[i].syntaxfunc(&buffer);
					 url = psprintf("https://www.raogarudb.io/docs/%s/%s.html",
								   strstr(PG_VERSION, "devel") ? "devel" : PG_MAJORVERSION,
								   QL_HELP[i].docbook_id);
					/* # of newlines in format must match constant above! */
					fprintf(output, _(
"######################################################################\n"
"RAOGARU DB Command Help\n"
"Command    : %s\n"
"Description: %s\n"
"URL        : %s\n\n"
"Syntax:\n%s\n"
"######################################################################\n\n"),
QL_HELP[i].cmd, _(QL_HELP[i].help), url, buffer.data);
					free(url);
...
{
	puts("RaogaruDB Database Management System\n\n");
}
